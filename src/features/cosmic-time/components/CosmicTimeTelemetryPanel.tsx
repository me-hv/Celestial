...
export const CosmicTimeTelemetryPanel: React.FC<CosmicTimeTelemetryPanelProps> = ({
  ...
  return (
    <div
      className="p-4 sm:p-6 rounded-2xl bg-celestial-surface/90 border border-white/[0.08] shadow-2xl backdrop-blur-2xl flex flex-col gap-4 sm:gap-5 text-celestial-starlight"
    ...
    >
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-white/[0.08] pb-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            />
            <span className="text-xs font-mono tracking-wider uppercase text-celestial-cyan font-semibold">
              EPOCH {epoch.orderIndex} OF 14 • {epoch.category.replace(/_/g, " ")}
            </span>
          </div>
          <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-mono">{epoch.name}</h2>
          <p className="text-xs text-celestial-subtle font-mono mt-0.5">{epoch.tagline}</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
        <div className="p-3 rounded-xl bg-celestial-deep/60 border border-white/[0.06]">
