...
export const CMBTelemetryPanel: React.FC<CMBTelemetryPanelProps> = ({ cmb }) => {
  return (
    <div
      className="flex flex-col gap-4 p-4 rounded-xl bg-slate-900/90 border border-slate-800 backdrop-blur-md font-mono"
    ...
    >
      <div className="flex items-start justify-between gap-2 border-b border-slate-800 pb-3">
        <div>
          ...
          <h3 className="text-base font-bold text-slate-100">{cmb.name}</h3>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
        <div className="p-2.5 rounded-lg bg-slate-950/60 border border-slate-800/60">
          ...
          <span className="text-orange-400 font-bold text-base">{cmb.redshiftZ.toFixed(1)}</span>
        </div>
        <div className="p-2.5 rounded-lg bg-slate-950/60 border border-slate-800/60">
          <span className="text-[10px] text-slate-500 block">Current Temp (T_0)</span>
          <span className="text-cyan-300 font-bold text-base">
            {cmb.temperatureKelvinToday.toFixed(4)} K
          </span>
        </div>
        <div className="p-2.5 rounded-lg bg-slate-950/60 border border-slate-800/60">
          <span className="text-[10px] text-slate-500 block">Cosmic Age (t)</span>
          <span className="text-purple-300 font-bold text-base">
            {(cmb.cosmicAgeYears / 1000).toFixed(0)} kyr
          </span>
        </div>
        <div className="p-2.5 rounded-lg bg-slate-950/60 border border-slate-800/60">
          <span className="text-[10px] text-slate-500 block">Comoving Dist.</span>
