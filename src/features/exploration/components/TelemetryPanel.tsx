...
export function TelemetryPanel({ object, onClose, onFocusCamera }: TelemetryPanelProps) {
  ...
  return (
    <aside
      ...
      className="absolute inset-x-3 bottom-20 top-auto sm:inset-x-auto sm:top-20 sm:bottom-auto sm:right-4 z-30 w-auto sm:w-full sm:max-w-sm max-h-[60vh] sm:max-h-[calc(100vh-14rem)] flex flex-col rounded-2xl border border-white/[0.1] bg-celestial-surface/95 backdrop-blur-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 sm:slide-in-from-right-4 duration-200"
    >
      {/* Header */}
